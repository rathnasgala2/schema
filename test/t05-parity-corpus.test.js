...
test('the committed snapshot locks each complete normalized result', async () => {
  // This is a same-language regression check (SCH-M17): it re-runs the JS
  // engine and compares against diagnostics/parity-expectations.json, which
  // was itself generated by the JS engine. It catches an accidental change
  // to validateStructuralCaseWithAjv's output; it is not cross-language
  // parity evidence on its own -- that comes only from `npm run
  // parity:check` (or `npm run verify`), which spawns the Java harness and
  // diffs its output against this same file. See README.md's `diagnostics/`
  // entry.
  const corpus = await createStructuralParityCases();
  const expectations = await loadParityExpectations();
  assert.deepEqual(
