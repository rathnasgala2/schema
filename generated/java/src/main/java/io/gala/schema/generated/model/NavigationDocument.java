// Generated from urn:gala:schema:navigation:2.0.0; sourceDesignRevision=a53052955ceb6c1aa10c886289dc832eedfe6de0a00004df07d353ac19c4403a.
package io.gala.schema.generated.model;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.JsonNode;
import java.util.List;
import java.util.Objects;

/** Closed generated record for urn:gala:schema:navigation:2.0.0. */
public record NavigationDocument(
        @JsonProperty("extensions") NavigationExtensionMap extensions,
        @JsonProperty("footerItems") List<NavigationNavigationItem> footerItems,
        @JsonProperty("items") List<NavigationNavigationItem> items,
        @JsonProperty("schemaId") String schemaId,
        @JsonProperty("schemaVersion") String schemaVersion
) {
    /** Reject a missing required root member before domain use. */
    public NavigationDocument {
        Objects.requireNonNull(extensions, "extensions");
        Objects.requireNonNull(footerItems, "footerItems");
        Objects.requireNonNull(items, "items");
        Objects.requireNonNull(schemaId, "schemaId");
        Objects.requireNonNull(schemaVersion, "schemaVersion");
    }
}
